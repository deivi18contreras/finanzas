...
const getEmoji = (nombre) => {
  ...
  return nuevaCategoria.value.tipo === 'ingreso' ? '💰' : '💸';
};
...
// Resuelve el emoji correcto para mostrar en la tarjeta
const resolverEmoji = (cat) => {
  const icono = cat.icono;
  if (!icono) return cat.tipo === 'ingreso' ? '💰' : '💸';
  // Si es un icon-name conocido de Material Design
  if (ICON_NAME_MAP[icono]) return ICON_NAME_MAP[icono];
  // Si ya es un emoji real (unicode, normalmente > 1 char en code points)
  const codePoint = icono.codePointAt(0);
  if (codePoint && codePoint > 127) return icono;
  // Intentar resolver por nombre de la categoría como fallback
...
const guardarCategoria = async () => {
  try {
    ...
    const emoji = getEmoji(nuevaCategoria.value.nombre);
